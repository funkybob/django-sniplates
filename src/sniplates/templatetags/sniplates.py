...
def parse_widget_name(widget):
    ...
    try:
        ...
        raise TemplateSyntaxError(f'widget name must be "alias:block_name" - {widget}')
...
def using(context, alias):
    ...
    if alias == '':
        ...
        try:
            ...
            raise TemplateSyntaxError('No widget libraries loaded!')

        try:
            block_set = widgets[alias]
        except KeyError:
            raise TemplateSyntaxError(f'No widget library loaded for alias: {alias !r}')
...
def find_block(context, *names):
    ...
    raise TemplateSyntaxError(f'No widget found for: {names !r}')
...
def nested_widget(parser, token):
    ...
    if bits:
        raise TemplateSyntaxError('{tag_name} accepts only one positional argument')
...
class FieldExtractor(dict):
    ...
    def initial(self):
        ...
        if callable(data):
            ...
            if (
                ...
                not self.form_field.widget.supports_microseconds
            ):
    ...
    @cached_property
    def choices(self):
        choices = self.form_field.field.choices
        if not choices:
            return choices
        return tuple(
...
class FileFieldExtractor(FieldExtractor):
    ...
    @cached_property
    def file_size(self):
        return self.raw_value.size if self.raw_value else None

    @cached_property
    def url(self):
        return self.raw_value.url if self.raw_value else None
...
class ImageFieldExtractor(FileFieldExtractor):
    ...
    @cached_property
    def width(self):
        return self.value.widget if self.value else None

    @cached_property
    def height(self):
        return self.value.height if self.value else None
...
def form_field(context, field, widget=None, **kwargs):
    if not field:
        raise TemplateSyntaxError('form_field requires a value field as first argument')
